...
public class HexagonalTable extends JPanel implements MouseMotionListener {
	...
	private void drawHex(Graphics g, int ox, int oy, int size, Color color, String text) {
		...
		int[] xPoints = { ox, (int) ((ox + (SQRT_3 / 2) * size)), (int) ((ox + SQRT_3 / 2 * size)), ox,
				(int) ((ox - SQRT_3 / 2 * size)), (int) ((ox - (SQRT_3 / 2) * size)) };
		int[] yPoints = { size + oy, (int) (0.5 * size + oy), (int) (oy - 0.5 * size), oy - 1 * size,
				(int) (oy - 0.5 * size), (int) (oy + 0.5 * size) };
		...
		/*
		 * g.setFont(g.getFont().deriveFont(12f)); FontMetrics fontMetrics =
		 * g.getFontMetrics(); int textWidth = fontMetrics.stringWidth(text); int
		 * textHeight = fontMetrics.getHeight(); int textX = ox - textWidth / 2; int
		 * textY = oy + textHeight / 4; // Draw the text g.drawString(text, textX,
		 * textY);
		 */
	}
